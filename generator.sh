...
mapfile -d '\' -t list < <(curl -fsSL https://github.com/Willie169/combinedfox/wiki/my-ublock-backup.txt | grep externalLists | sed 's/^[[:space:]]*"externalLists":[[:space:]]*"//; s/",[[:space:]]*$//')
for l in "${list[@]}"; do
...
! Description: all-in-one filterlist combining all lists used in https://github.com/Willie169/combinedfox/wiki/my-ublock-backup.txt, designed for situations where using single list is the only option or more convenient, such as Content Filtering of Brave for Android
